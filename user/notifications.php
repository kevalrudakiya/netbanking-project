...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_POST['csrf_token']) || !validateCsrfToken($_POST['csrf_token'])) {
        ...
        if ($action === 'mark_read') {
            $id = (int)$_POST['id'];
            $stmt = mysqli_prepare($conn, "UPDATE notifications SET is_read=1 WHERE id=? AND user_id=? AND type='user'");
            mysqli_stmt_bind_param($stmt, 'ii', $id, $user_id);
            mysqli_stmt_execute($stmt);
        } elseif ($action === 'delete') {
            $id = (int)$_POST['id'];
            $stmt = mysqli_prepare($conn, "DELETE FROM notifications WHERE id=? AND user_id=? AND type='user'");
            mysqli_stmt_bind_param($stmt, 'ii', $id, $user_id);
            mysqli_stmt_execute($stmt);
            $success = "Notification deleted.";
        } elseif ($action === 'mark_all_read') {
            $stmt = mysqli_prepare($conn, "UPDATE notifications SET is_read=1 WHERE user_id=? AND type='user'");
...
// Fetch
$stmt = mysqli_prepare($conn, "SELECT * FROM notifications WHERE type='user' AND (user_id=? OR user_id=0) ORDER BY created_at DESC");
mysqli_stmt_bind_param($stmt, 'i', $user_id);
mysqli_stmt_execute($stmt);
$result = mysqli_stmt_get_result($stmt);

// Define active for sidebar if it existed, but we just set it.
$active = 'notifications'; // not actually in sidebar, but harmless
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="dashboard-container">
    ...
    <main class="main-content">
        <header class="page-header" style="display:flex; justify-content:space-between; align-items:center;">
            <div>
                <h1><i class="fa-regular fa-bell" style="color: var(--accent-blue);"></i> Notifications</h1>
            ...
            <div class="notif-card <?= $n['is_read'] ? '' : 'unread' ?>">
                <div class="notif-content">
                    <h4>
                        ...
                        <?php if ($n['user_id'] == 0): ?>
                            <span class="badge-broadcast">Announcement</span>
                        <?php endif; ?>
                    </h4>
