<?php
// Expects $active to be set by the including page.
$active = $active ?? '';

$navItems = [
    'dashboard'  => ['dashboard.php',         'fa-chart-pie',       'Dashboard'],
    'users'      => ['users.php',             'fa-users',           'Users'],
    'accounts'   => ['accounts.php',          'fa-building-columns','Accounts'],
    'transactions'=> ['transactions.php',     'fa-money-bill-wave', 'Transactions'],
    'requests'   => ['password_requests.php', 'fa-unlock-keyhole',  'Password Requests'],
    'audit'      => ['audit_logs.php',        'fa-list-check',      'Audit Logs'],
    'password'   => ['change_password.php',   'fa-key',             'Change Password'],
];
?>
<aside class="sidebar">
    <div class="brand" style="display: flex; justify-content: space-between; align-items: center; width: 100%;">
        <div><i class="fa-solid fa-shield-halved" style="color: var(--primary-glow);"></i> Secure<span>Bank</span></div>
        <div class="notif-bell" style="position: relative;">
            <?php 
                $unread = getUnreadNotificationCount($conn, $_SESSION['admin_id'] ?? 0, 'admin'); 
            ?>
            ...
            <div class="notif-dropdown">
                <?php 
                    ...
                    $ndrop = mysqli_query($conn, "SELECT * FROM notifications WHERE type='admin' AND (user_id=$uid OR user_id=0) ORDER BY created_at DESC LIMIT 3");
    ...
    <div style="font-size: 11px; text-transform: uppercase; letter-spacing: 1px; color: var(--text-muted); margin: -30px 0 25px 4px;">Admin Panel</div>
    <ul class="nav-menu">
